...
class Flx_Config extends CI_Config {
    ...
    function __construct()
    {
        ...
        else
        {
            ...
            $this->set_item('base_str', $this->config['base_url']);
            $this->set_item('base_url', $base_url);
        }
    }
  ...
  */
  ...
  {
    $res_url = isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) !== 'off' ? 'https' : 'http';
    $res_url .= '://'.$this->slash_item('base_str').trim($this->_uri_string($uri), '/');
    return rtrim($res_url, '/');
  } 
