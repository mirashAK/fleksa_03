...
//     }
  ...
  protected function row_array (&$user, $table, $where = '')
  {
    ...
    if (!empty($first_result) && $first_result->num_rows() == 1)
    {
      ...
      foreach ($first_result as $key=>$value)
      {
        ...
        $final_result['length'][$key] = $value['length'];
      }
  ...
  protected function row_object ($user, $table, $where = '')
  {
    ...
    if (!empty($first_result) && $first_result->num_rows() == 1)
    {
      ...
      $final_result['length']= (object)$value['length'];
  ...
  protected function full_objects (&$user, $table, $where = '', $order = '', $limit = '')
  {
    ...
    if (!empty($first_result) && $first_result->num_rows() == 1)
    {
      ...
      $final_result['require'] = (object)$final_result['require'];
      $final_result['unique'] = (object)$final_result['unique'];
      $final_result['length'] = (object)$final_result['length'];
  ...
  {
    $sql = 'CALL set_table(?,?,?,?,?)';
    $table_id = $form->get_id_field();
    if(empty($where)) $where = "`{$table_id}`= {$form->$table_id}";
    else $where .= "AND `{$table_id}`= {$form->$table_id}";
    
    foreach ($form->get_unique() as $field=>$is_uniq)
    { 
      if ($is_uniq == true)
      {
        $is_pass = $this->check_unique($table, array($field => $form->$field), array('field' => $table_id, 'value' => $form->$table_id));
        if ($is_pass == false)
        {
          $form->add_error($field, 'not unique');
          return false;
        }
      }
    }
  ...
  public function check_unique ($table, $condition, $table_id = array())
  {
    $sql = 'CALL check_uniq(?,?)';
    ...
    if (!empty($table_id)) $result_condition .= '`'.$table_id['field']."`<> '".$table_id['value']."'";
    else $result_condition = substr ($result_condition, 0, strlen($result_condition)-5);
