...
--
-- Хост: 127.0.0.1:3306
-- Час створення: Трв 24 2013 р., 17:08
...
BEGIN
	...
	IF (var_user_id IS NULL) THEN
		...
		INSERT INTO `syst_sessions` (`sess_user_special`, `sess_user_token`, `sess_user_data`, `sess_user_last_activity`)
		VALUES ('registration', var_user_code, in_pass, NOW());
...
CREATE TABLE IF NOT EXISTS `syst_sessions` (
  ...
  `sess_user_special` enum('normal','registration','recovery') NOT NULL DEFAULT 'normal',
...
(1, '0:05:0.0', '2013-05-24 12:58:20', '0:01:0.0', 'ababagalamaga');
