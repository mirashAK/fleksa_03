...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Май 11 2014 г., 14:43
...
--
-- База данных: `fleksa03_test`
--
CREATE DATABASE IF NOT EXISTS `fleksa03_test` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `fleksa03_test`;
...
BEGIN
	...
	DECLARE var_length VARCHAR(5) DEFAULT '';
	...
	WHILE var_is_end != true DO
		...
		SELECT `CHARACTER_MAXIMUM_LENGTH` INTO var_length FROM `information_schema`.`columns` 
		WHERE `table_schema` = DATABASE() AND `TABLE_NAME` = in_table_name AND `COLUMN_NAME` = var_column;
		IF (var_length IS NULL) THEN SET var_length = 'false'; ELSE SET var_length = CAST(var_length as SIGNED); END IF;

		SET out_defs = CONCAT
		(out_defs,'"',var_column,
		'":{"type":"',var_def,'","caption":"',var_caption,'","r_only":',var_ro_ind,
		',"require":',var_req,',"unique":',var_uniq,',"length":', var_length,
		'},');
...
BEGIN
    ...
    ELSE
        ...
        WHEN var_user_id > 0 THEN
            ...
            SELECT in_user_token AS 'user_token', in_ip AS 'last_ip', var_user_auth_time AS 'last_activity',
            var_user_id AS 'user_id', var_user_data AS 'session_data',
            `usr`.`user_login` AS 'user_login', `usr`.`user_email` AS 'user_email', `usr`.`user_is_active` AS 'is_active',
			`usr`.`user_f_group_id` AS 'group_id', `usr`.`user_reg_date` AS 'reg_date'
...
--
-- Структура таблицы `dict_genders`
--
...
--
-- Дамп данных таблицы `dict_genders`
--
...
--
-- Структура таблицы `dict_timesets`
--
...
--
-- Дамп данных таблицы `dict_timesets`
--
...
--
-- Структура таблицы `hrefs_articles_to_categories`
--
...
--
-- Дамп данных таблицы `hrefs_articles_to_categories`
--
...
--
-- Структура таблицы `public_articles`
--
...
--
-- Дамп данных таблицы `public_articles`
--
...
--
-- Структура таблицы `public_categories`
--
...
--
-- Дамп данных таблицы `public_categories`
--
...
--
-- Структура таблицы `public_static_pages`
--
...
CREATE TABLE IF NOT EXISTS `public_static_pages` (
  ...
  `static_page_alias` varchar(5) NOT NULL,
...
--
-- Дамп данных таблицы `public_users`
--
...
(1, 1, NULL, NULL, NULL, NULL, NULL, 10, 41, 1),
(2, 2, NULL, NULL, NULL, NULL, NULL, 10, 41, 2),
(4, 4, NULL, NULL, NULL, NULL, NULL, 10, 41, 4),
(5, 5, NULL, NULL, NULL, NULL, NULL, 10, 41, 5),
(6, 6, NULL, NULL, NULL, NULL, NULL, 10, 41, 6);

-- --------------------------------------------------------

--
-- Структура таблицы `syst_column_types`
--
...
--
-- Дамп данных таблицы `syst_column_types`
--
...
--
-- Структура таблицы `syst_defs_columns`
--
...
--
-- Дамп данных таблицы `syst_defs_columns`
--
...
(5, 'public_menus', 'owner', 'id', NULL, 0, 0, 'owner', 80, 50, 0),
(6, 'public_static_pages', 'static_page_id', 'id', NULL, 0, 0, 'id', 20, 50, 0),
(7, 'public_static_pages', 'static_page_alias', 'edit', NULL, 1, 1, 'alias', 0, 0, 0),
(8, 'public_static_pages', 'static_page_text', 'text', NULL, 0, 0, 'text', 0, 0, 0),
...
(13, 'public_categories', 'cat_parent_id', 'f_id', NULL, 0, 0, 'parent_id', 0, 80, 0),
...
(18, 'hrefs_articles_to_categories', 'href_cat_id', 'f_id', NULL, 0, 0, 'cat_id', 0, 50, 0),
(19, 'hrefs_articles_to_categories', 'href_article_id', 'f_id', NULL, 0, 0, 'article_id', 0, 50, 0),
...
(46, 'public_users', 'u_f_user_id', 'f_id', NULL, 0, 1, NULL, 0, 40, 2),
(53, 'public_users', 'u_timeset', 'select', 'dict_timesets', 0, 0, 'Timezones', 0, 20, 7);

-- --------------------------------------------------------

--
-- Структура таблицы `syst_defs_tables`
--
...
--
-- Дамп данных таблицы `syst_defs_tables`
--
...
(6, 'public_static_pages', 0, 50, 40, 50);

-- --------------------------------------------------------

--
-- Структура таблицы `syst_groups`
--
...
--
-- Дамп данных таблицы `syst_groups`
--
...
--
-- Структура таблицы `syst_sessions`
--
...
--
-- Дамп данных таблицы `syst_sessions`
--
...
(82, 0, 'ac33c6828d0e057cba0e8e9d0a342c98', '127.0.0.1', 'ikJychDgZbSL3w0gGmGc7Liz7WhsXUjAw86U5ByVlzAjcn8qk9O69ge+64TVZbMiSyPvy1711DLoEqFVquCPGA', '2014-05-06 12:01:07');

-- --------------------------------------------------------

--
-- Структура таблицы `syst_settings`
--
...
--
-- Дамп данных таблицы `syst_settings`
--

INSERT INTO `syst_settings` (`sett_id`, `sett_session_time`, `sett_session_last`, `sett_user_token_time`, `sett_passw_salt`) VALUES
(1, '2:15:0.0', '2014-05-06 12:00:14', '1:05:0.0', 'ababagalamaga');

-- --------------------------------------------------------

--
-- Структура таблицы `syst_users`
--
...
--
-- Дамп данных таблицы `syst_users`
--

INSERT INTO `syst_users` (`user_id`, `user_login`, `user_email`, `user_pass`, `user_is_active`, `user_f_group_id`, `user_reg_date`) VALUES
(1, '', 'bers_777@ukr.net', '763259646ecb5234653b19809bacc972', 1, 4, '2013-12-24 17:23:00'),
(2, '', 'evgen302@i.ua', '763259646ecb5234653b19809bacc972', 1, 4, '2013-12-24 17:29:08'),
(3, '', 'evgen300@i.ua', '763259646ecb5234653b19809bacc972', 1, 4, '2013-12-24 17:35:43'),
(4, '', 'evgen304@i.ua', '763259646ecb5234653b19809bacc972', 1, 4, '2013-12-25 09:37:11'),
(5, '', 'evgen301@i.ua', '763259646ecb5234653b19809bacc972', 1, 4, '2014-01-06 16:31:52'),
(6, '', 'evgen303@i.ua', '763259646ecb5234653b19809bacc972', 1, 4, '2014-01-06 16:37:08');
