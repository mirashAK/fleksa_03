...
--
-- Хост: 127.0.0.1:3306
-- Час створення: Трв 20 2013 р., 17:54
...
BEGIN
	...
	/* Clear old sessions */
	SELECT clear_sessions('') INTO var_garbage;
	...
	ELSE
		IF (in_user_token = '') THEN
			...
			INSERT INTO `syst_sessions` (`sess_user_token`,`sess_user_last_ip`,`sess_user_last_activity`)
			VALUES (in_user_token, in_ip, NOW());
		END IF;

		/* Try to get user session data */
		SELECT `sess_id`, `sess_user_last_ip`, `sess_user_last_activity`, `sess_f_user_id`, `sess_user_data`
		INTO var_sess_id, var_user_last_ip, var_user_auth_time, var_user_id, var_user_data
		FROM `syst_sessions` 
    ...
    CASE
		...
		WHEN var_sess_id IS NULL THEN
			...
			SELECT '0' AS 'Status';
			SELECT in_user_token AS 'user_token', in_ip AS 'last_ip', NOW() AS 'last_activity',
       ...
       WHEN var_user_id > 0 THEN
			...
			IF (var_user_last_ip != in_ip)||(var_user_is_active IS NULL) THEN

				/* Update last user activity */
				UPDATE `syst_sessions` SET `sess_user_last_activity` = NOW(), `sess_user_last_ip` = in_ip, `sess_f_user_id` = 0 
				WHERE `sess_user_token` = in_user_token LIMIT 1;

				SELECT '0' AS 'Status';
				SELECT in_user_token AS 'user_token', in_ip AS 'last_ip', var_user_auth_time AS 'last_activity',
				0 AS 'user_id', var_user_data AS 'session_data';
...
CREATE DEFINER=`root`@`%` FUNCTION `clear_sessions`(`in_user_token` VARCHAR(40)) RETURNS varchar(255) CHARSET utf8
    READS SQL DATA
BEGIN
	DECLARE var_token_life_time VARCHAR(20);
	DECLARE var_token_last_time VARCHAR(20);
	...
	/* Get settings */
	SELECT `sett_session_time`,`sett_session_last`,`sett_user_token_time`,`sett_user_token_last`
	INTO var_session_life_time, var_session_last_time, var_token_life_time, var_token_last_time
    FROM `syst_settings` WHERE `sett_id` = 1 LIMIT 1;


	/* Calculate time */
	SET var_session_life_time = UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(ADDTIME(var_session_last_time, var_session_life_time));
	SET var_session_life_time = CAST(var_session_life_time as SIGNED);

	SET var_token_life_time = UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(ADDTIME(var_token_last_time, var_token_life_time));
	SET var_token_life_time = CAST(var_token_life_time as SIGNED);

	IF (var_session_life_time > 0) THEN
		DELETE FROM `syst_sessions` WHERE `sess_user_last_activity` < var_session_last_time;
		UPDATE `syst_settings` SET `sett_session_last` = NOW() WHERE `sett_id` = 1 LIMIT 1;
	END IF;

	IF (var_token_life_time > 0) THEN 
		UPDATE `syst_sessions` SET `sess_f_user_id` = 0 WHERE `sess_user_last_activity` < var_token_last_time;
		UPDATE `syst_settings` SET `sett_user_token_last` = NOW() WHERE `sett_id` = 1 LIMIT 1;
	END IF;

	IF (in_user_token != '') THEN 
		UPDATE `syst_sessions` SET `sess_f_user_id` = 0 WHERE `sess_user_token` = in_user_token;
	END IF;
	
RETURN CONCAT('token_last_time:',var_token_last_time,' token_life_time:', var_token_life_time,' session_last_time:',var_session_last_time,' session_life_time:',var_session_life_time);
END$$
...
BEGIN
	...
	IF (exist_token != '') THEN
		...
		DELETE FROM `syst_sessions` WHERE `sess_f_user_id` = var_user_id LIMIT 1;
...
(93, 0, '4029150a40de838748b8e0d530bea3a1', '192.168.0.2', 'O:7:"MyClass":2:{s:3:"foo";i:10;s:3:"bar";i:20}', '2013-05-20 12:54:05'),
(97, 3, '92c5c6f93a9f1fbb5b1d6eda6613e9dd', '127.0.0.1', 'O:7:"MyClass":2:{s:3:"foo";i:10;s:3:"bar";i:20}', '2013-05-20 13:25:28');
...
CREATE TABLE IF NOT EXISTS `syst_settings` (
  ...
  `sett_user_token_last` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00',
...
INSERT INTO `syst_settings` (`sett_id`, `sett_session_time`, `sett_session_last`, `sett_user_token_time`, `sett_user_token_last`, `sett_passw_salt`) VALUES
(1, '0:02:0.0', '2013-05-20 13:23:31', '0:01:0.0', '2013-05-20 13:25:28', 'ababagalamaga');
...
(3, 'test', 'test@mail', 'df7f48cfa90519dc72aa72c9c8e869b3', 1, 3, '0000-00-00 00:00:00');
